...
if TYPE_CHECKING:
    from base_class.data_formats.root import Friend
...
class WorkerInitializer(WorkerPlugin):
    ...
    def setup(self, worker=None):
        if delays := self.uproot_xrootd_retry_delays:
            from base_class.data_formats.root.patch import uproot_XRootD_retry
...
def setup_config_defaults(config_runner, args):
    ...
    defaults = {
        ...
        'condor_cores': 2,
        'condor_memory': '4GB',
    'condor_transfer_input_files': ['analysis/', 'base_class/', 'classifier/', 'skimmer/'],
        'min_workers': 1,
        'max_workers': 100,
        'workers': 2,
        'skipbadfiles': False,
...
def process_friend_trees(output, config_runner, configs, args, client):
    ...
    if friend_base is not None and friends is not None:
        from base_class.data_formats.awkward.zip import NanoAOD
        ...
        from base_class.storage.eos import EOS
...
if __name__ == '__main__':
    ...
    io_group.add_argument(
        '-p', '--processor',
        dest="processor",
        default="analysis/processors/processor_HH4b.py",
    ...
    with open('base_class/physics/corrections.yml', 'r') as f:
    ...
    data_group.add_argument(
        ...
        help=f"Year(s) of data to process (as in base_class/physics/corrections.yml). Choices: {', '.join(year_choices)}. Examples: --years UL17 UL18"
    )
    data_group.add_argument(
        '-d', '--datasets',
        nargs='+',
    ...
    logging.info("Loading corrections metadata from: base_class/physics/corrections.yml")
