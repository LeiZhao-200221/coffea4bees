...
if [[ $(hostname) = *fnal* ]]; then
    sed -e "s#base_path.*#base_path: /srv/${OUTPUT_DIR}/#" -e "s/\#max.*/maxchunks: 5/" -e "s/\#test.*/test_files: 1/" -e "s/2024_.*/tmp\//" coffea4bees/skimmer/metadata/HH4b_boosted.yml > $OUTPUT_DIR/HH4b_boosted.yml
else
    sed -e "s#base_path: /srv.*#base_path: /builds/${CI_PROJECT_PATH}/${OUTPUT_DIR}/#" -e "s/\#max.*/maxchunks: 5/" -e "s/\#test.*/test_files: 1/" -e "s/2024_.*/tmp\//" -e "s/T3_US_FNALLPC/T3_CH_PSI/" coffea4bees/skimmer/metadata/HH4b_boosted.yml > $OUTPUT_DIR/HH4b_boosted.yml
fi
...
python runner.py -s -p coffea4bees/skimmer/processor/skimmer_4b_boosted.py -c $OUTPUT_DIR/HH4b_boosted.yml -y UL18 -d GluGluToHHTo4B_cHHH0 -op $OUTPUT_DIR -o picoaod_datasets_GluGluToHHTo4B_cHHH0_UL18.yml -m $OUTPUT_DIR/datasets_HH4b.yml  -t 
ls -R $OUTPUT_DIR
